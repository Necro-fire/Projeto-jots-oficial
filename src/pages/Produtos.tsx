...
export default function Produtos() {
  ...
  const [zoomImage, setZoomImage] = useState<{ url: string; name: string; category?: string; classificacao?: string } | null>(null);
                ...
                <div key={product.id} className="rounded-lg shadow-card bg-card p-3 group hover:shadow-md transition-shadow relative">
                    <div className="absolute top-2 right-2 z-10 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {canViewImages && product.image_url && (
                        <Button variant="secondary" size="icon" className="h-7 w-7" title="Visualizar" onClick={() => setZoomImage({ url: product.image_url, name: product.model || product.referencia, category: product.category, classificacao: (product as any).classificacao })}>
